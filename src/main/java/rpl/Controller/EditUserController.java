...
public class EditUserController implements Initializable {
    ...
    @FXML
    private TextField userchangetxt,emailchangetxt,namachangetxt;
...
//        hm.setSaldotxt(saldotxt.getText());
    ...
    public void simpan(ActionEvent ae){
        String user = userchangetxt.getText();
        ...
        conn=konek.conDB();
        st=conn.createStatement();
        if((user.isEmpty() || user==" ") && (nama.isEmpty() || nama==" ") && (email.isEmpty()||email==" ")){
            JOptionPane.showMessageDialog(null, "FIELD TIDAK BOLEH KOSONG SEMUA");
        }else if((!user.isEmpty() ) && (!nama.isEmpty()) && (!email.isEmpty())){
            st.executeUpdate("update user set email='"+email+"',username='"+user+"',nama_lengkap='"+nama+"' where username='"+this.usernametxt.getText()+"'");
            this.usernametxt.setText(user);
            JOptionPane.showMessageDialog(null, "Berhasil ganti profil");
            kembalikehome();
        }else if((user.isEmpty()) && (!nama.isEmpty()) && (email.isEmpty())){
            st.executeUpdate("update user set nama_lengkap='"+nama+"' where username='"+this.usernametxt.getText()+"'");
            JOptionPane.showMessageDialog(null, "Berhasil ganti profil");
            kembalikehome();
        }else if((user.isEmpty()) && (!nama.isEmpty()) && (!email.isEmpty())){
            st.executeUpdate("update user set email='"+email+"',nama_lengkap='"+nama+"' where username='"+this.usernametxt.getText()+"'");
            JOptionPane.showMessageDialog(null, "Berhasil ganti profil");
            kembalikehome();
        }else if((!user.isEmpty()) && (nama.isEmpty()) && (email.isEmpty())){
            st.executeUpdate("update user set username='"+user+"' where username='"+this.usernametxt.getText()+"'");
            this.usernametxt.setText(user);
            JOptionPane.showMessageDialog(null, "Berhasil ganti profil");
            kembalikehome();
        }else if((!user.isEmpty()) && (!nama.isEmpty()) && (email.isEmpty())){
            st.executeUpdate("update user set username='"+user+"',nama_lengkap='"+nama+"' where username='"+this.usernametxt.getText()+"'");
            this.usernametxt.setText(user);
            JOptionPane.showMessageDialog(null, "Berhasil ganti profil");
            kembalikehome();
        }else if((!user.isEmpty()) && (nama.isEmpty()) && (!email.isEmpty())){
            st.executeUpdate("update user set username='"+user+"',email='"+email+"' where username='"+this.usernametxt.getText()+"'");
            this.usernametxt.setText(user);
            JOptionPane.showMessageDialog(null, "Berhasil ganti profil");
            kembalikehome();            
        }else if((user.isEmpty()) && (nama.isEmpty()) && (!email.isEmpty())){            
            st.executeUpdate("update user set email='"+email+"' where username='"+this.usernametxt.getText()+"'");
            JOptionPane.showMessageDialog(null, "Berhasil ganti profil");
            kembalikehome();            
        }else if((!user.isEmpty()) && (nama.isEmpty()) && (!email.isEmpty())){
            st.executeUpdate("update user set username='"+user+"',email='"+nama+"' where username='"+this.usernametxt.getText()+"'");            
            this.usernametxt.setText(user);
            JOptionPane.showMessageDialog(null, "Berhasil ganti profil");
            kembalikehome();
